-- ddl-cubrid.sql
-- Wikipedia schema for CUBRID 11.x.
--
-- Derived from ddl-generic.sql by substituting three types CUBRID does not
-- have. Nine of the twelve tables fail without this; only useracct,
-- value_backup and watchlist are accepted as written.
--
--   varbinary(n) -> VARCHAR(n)   ipblocks, logging, page_restrictions,
--                                recentchanges, user_groups
--   binary(n)    -> CHAR(n)      page, page_backup, revision
--   TEXT         -> STRING       text
--
-- VARCHAR rather than BIT VARYING(n*8) for the binary columns: MediaWiki
-- declares them binary, but WikipediaLoader binds them with setString -- 28
-- calls, no setBytes anywhere -- so they carry text, and a character type is
-- what the loader actually needs. CUBRID counts VARCHAR in characters where
-- varbinary counts bytes, so a multi-byte value could occupy fewer characters
-- than the declared byte width allowed; nothing in the generated data comes
-- close to these limits.
--
-- Everything else is accepted unchanged. tinyint in particular is fine -- bare
-- tinyint is a CUBRID type, and this file never writes tinyint(1), which is
-- the form CUBRID rejects. The auto_increment mentioned in the comments is
-- only ever a TODO; no column here is generated.
