-- ddl-cubrid.sql
-- TPC-C schema for CUBRID 11.x.
--
-- Derived from ddl-postgres.sql. CUBRID compatibility notes:
--   - timestamp: CUBRID TIMESTAMP is identical in semantics to Postgres for
--     the precision used here (no TIMESTAMP WITHOUT TIME ZONE needed).
--   - decimal(p,s), varchar(n), char(n), float: all supported natively.
--   - No USING clause on CREATE INDEX (CUBRID default is already btree).
--   - No SERIAL or AUTO_INCREMENT columns in the TPC-C schema; none needed.
--   - ON DELETE CASCADE on FOREIGN KEY: supported.
--   - DEFAULT CURRENT_TIMESTAMP: supported.
--
-- Phase 1 note (ADR-2): schema is created by BenchBase client-side loader;
-- no stored procedures are deployed in Phase 1.

DROP TABLE IF EXISTS history CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS new_order CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS order_line CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS oorder CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS customer CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS district CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS stock CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS item CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS warehouse CASCADE CONSTRAINTS;
