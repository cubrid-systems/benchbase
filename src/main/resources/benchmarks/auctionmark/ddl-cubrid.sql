-- ddl-cubrid.sql
-- AuctionMark schema for CUBRID 11.x.
--
-- Derived from ddl-generic.sql with exactly one change, in CREATE TABLE
-- category. The generic file declares the self-referencing foreign key inline,
-- on the column, above the PRIMARY KEY line:
--
--     c_parent_id BIGINT REFERENCES category (c_id),
--     PRIMARY KEY (c_id)
--
-- CUBRID resolves the reference at the point the foreign key is declared, so at
-- that moment category has no primary key yet and the statement fails with
--   ERROR: The class 'dba.category' referred by the foreign key
--          does not have the primary key.
-- Every later table references category, so the whole file fails behind it.
--
-- The fix is the one already used for TPC-C's new_order: declare the primary
-- key first and attach the foreign key as a table-level constraint after it.
-- No other table needs that reordering -- their foreign keys point at tables
-- created earlier in the file, whose primary keys therefore already exist.
--
-- The second change is in item_max_bid, and is the same fix new_order already
-- carries: its FOREIGN KEY (imb_i_id, imb_u_id) covers exactly the columns of
-- its PRIMARY KEY. Declared foreign-key-first, CUBRID builds the foreign key's
-- index before the primary key's and then rejects the second as a duplicate:
--   ERROR: Index "fk_item_max_bid_imb_i_id_imb_u_id" already defined
--          for class "dba.item_max_bid".
-- Declaring the primary key first lets CUBRID reuse its index for the foreign
-- key. A scan of the file found item_max_bid to be the only table where a
-- foreign key and the primary key cover identical column lists.
...
CREATE TABLE category (
    ...
    c_parent_id         BIGINT,
    PRIMARY KEY (c_id),
    FOREIGN KEY (c_parent_id) REFERENCES category (c_id)
);
