-- TPC-C consistency conditions 1-4 (TPC-C v5.11 §3.3.2) — MySQL port.
-- Reference: BenchBase loaded `tpcc` at SF=1 on MySQL.
-- A passing run reports `0` (or empty) for every violation query below.
-- Schema note: BenchBase on MySQL uses table `oorder` (same as CUBRID).
-- Dialect note: MySQL's `||` is boolean-OR by default; use CONCAT() for string concat.

-- Condition 1: W_YTD = sum(D_YTD) for each warehouse.
SELECT CONCAT('COND1 violations: ', COUNT(*))
FROM (
    SELECT w.w_id,
           w.w_ytd,
...
SELECT CONCAT('COND2 violations: ', COUNT(*))
FROM district d
...
SELECT CONCAT('COND3 violations: ', COUNT(*))
FROM (
    SELECT n.no_w_id,
           n.no_d_id,
...
SELECT CONCAT('COND4 violations: ', COUNT(*))
FROM (
    SELECT o.o_w_id,
           o.o_d_id,
